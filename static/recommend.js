$(function() {
    ...
    $('.movie-button').on('click', function() {
        var my_api_key = "4098ac422f2f450ff1da79b9665f7299";
...
// will be invoked when the user clicks on one of the recommended movies
...
// get the details of the movie from the API using the title
...
// passing the movie title to get the similar movies from our Flask API 
function movie_recs(movie_title, movie_id, my_api_key) {
    $.ajax({
        ...
        success: function(recommendations) {
            if (recommendations == "Sorry! The movie you requested is not in our database. Please check the spelling or try with some other movies") {
                $('.fail').css('display', 'block');
                $('.results').css('display', 'none');
                $("#loader").delay(500).fadeOut();
            } else {
                ...
                var movie_arr = recommendations.split('---');
                var arr = [];
...
// passing all the details to the Flask API for scraping and displaying the movie reviews using imdb id 
...
// get the details of individual cast memebers from TMDb API using cast_id
...
// getting posters for all the recommendations using the TMDb API
